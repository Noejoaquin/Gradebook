...
class CourseProfile extends React.Component {
  ...
  createStudentIndex(students) {
    return students.map((student, i) => {
      return (
        <li
          onClick={() => this.selectViewPort(student.id)}
          className="profile-index-item student-index-item"
          key={student.id}
        >
  ...
  render() {
    ...
    if (this.props.students && this.props.course && this.props.currentUser) {
      ...
      teacher =
        this.props.currentUser.role === "admin" ? (
          <h1 className="teacher">Teacher : {this.props.course.teacher}</h1>
        ) : (
          <h1 />
        );
    ...
    if (this.state.content === "") {
      emptyViewPortMessage = (
        <li id="empty-message" className="empty-view-port-message">
    ...
    return (
      <div className="course-profile-container">
        ...
        <div className="index-port-container">
          <ul className="profile-index student-index">{students}</ul>
