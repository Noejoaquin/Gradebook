...
class StudentProfile extends React.Component {
  ...
  makeClassList() {
    ...
    return this.props.student.courses.map(course => {
      grade = this.props.student.grades.filter(
        grade => grade.course_id === course.id
      )[0];
      return (
        <ul
          key={course.id}
          className="profile-index-item student-profile-index-item"
        >
          <li className="student-course-name" key={course.id}>
            {course.name}
          </li>
  ...
  render() {
    ...
    if (this.props.student && this.props.student.courses) {
      ...
      gpa = (
        <h2 className="student-gpa">Overall Gpa: {this.props.student.gpa}</h2>
      );
    }
    return (
      <div className="student-profile-container">
        ...
        <ul className="profile-container course-list-container">{classList}</ul>
      </div>
    );
  }
}
